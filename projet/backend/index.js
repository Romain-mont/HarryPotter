...
app.get("/subjects", (req, res) => {
  ...
  client.query("SELECT * FROM subject ORDER BY name ASC")
    .then((result) => {
...
app.get("/subjects/:id", (req, res) => {
  ...
  client.query("SELECT * FROM subject WHERE id=$1", [id])
    .then((result) => {
...
app.post("/login", (req, res) => {
  ...
  client.query("SELECT * FROM wizard WHERE email=$1 AND password=$2", [email, password])
    .then((result) => {
...
app.get("/nav-links", async (req, res) => {
  ...
  let decoded = null;
  try {
    if ( token ) decoded = await jwt.verify(token.split(" ")[1], privatekey);
  } catch (error) {
    return res.status(500).json({ message: error });
  }
...
app.get("/wizard-roles", checkWizardToken, (req, res) => {
  ...
  client.query("SELECT Role.* FROM WizardRole JOIN Role ON WizardRole.role_id = Role.id WHERE wizard_id=$1 ORDER BY Role.name", [wizard_id])
    .then((result) => {
      res.json(result.rows);
    })
    .catch(error => {
...
app.post("/login-role", checkWizardToken, (req, res) => {
  ...
  `
  client.query(query, [wizard.id, role_id])
    .then((result) => {
  ...
  client.query(query)
    .then((result) => {
      ...
      result.forEach(wizard => {
        if (!unique_wizards.find(w => w.id === wizard.id))  unique_wizards.push(wizard);
      });
...
app.get("/wizards/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("SELECT * FROM wizard WHERE id=$1", [id])
    .then((result) => {
      const wizard = result.rows[0];
      delete wizard.password;
      res.json(wizard);
...
app.put("/wizards/:id", (req, res) => {
  ...
  client.query(
    ...
    [wizard.lastname, wizard.firstname, wizard.birthdate, wizard.image, wizard.house_id, wizard.class_id, wizard.email, id]
  ).then((result) => {
...
app.delete("/wizards/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("DELETE FROM wizard WHERE id=$1", [id])
    .then(() => {
      res.status(204).json();
    })
    .catch(error => {
...
app.get("/roles", (req, res) => {
  ...
  client.query("SELECT * FROM role ORDER BY name ASC")
    .then((result) => {
      res.json(result.rows);
    })
    .catch(error => {
...
app.get("/roles/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("SELECT * FROM role WHERE id=$1", [id])
    .then((result) => {
      res.json(result.rows[0]);
    })
    .catch(error => {
...
app.post("/roles", (req, res) => {
  ...
  const role = req.body;
  client.query(
    "INSERT INTO role (name, is_staff) VALUES ($1, $2) RETURNING *",
    [role.name, role.is_staff]
  ).then((result) => {
    res.status(201).json(result.rows[0]);
  })
  .catch(error => {
...
app.put("/roles/:id", (req, res) => {
  ...
  const role = req.body;
  client.query(
    "UPDATE role SET name=$1, is_staff=$2 WHERE id=$3 RETURNING *",
    [role.name, role.is_staff, id]
  ).then((result) => {
    res.status(200).json(result.rows[0]);
  })
  .catch(error => {
...
app.post("/subjects", (req, res) => {
  ...
  client.query(
    ...
    [subject.name, subject.description, subject.image, subject.room_id, subject.wizard_id]
  ).then((result) => {
    res.status(201).json(result.rows[0]);
  })
  .catch(error => {
...
app.put("/subjects/:id", (req, res) => {
  ...
  client.query(
    ...
    [subject.name, subject.description, subject.image, subject.room_id, subject.wizard_id, id]
  ).then((result) => {
    res.status(200).json(result.rows[0]);
  })
  .catch(error => {
...
app.delete("/subjects/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("DELETE FROM subject WHERE id=$1", [id])
    .then(() => {
      res.status(204).json();
    })
    .catch(error => {
...
app.get("/classes", (req, res) => {
  ...
  client.query("SELECT * FROM class ORDER BY name ASC")
    .then((result) => {
      res.json(result.rows);
    })
    .catch(error => {
...
app.get("/classes/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("SELECT * FROM class WHERE id=$1", [id])
    .then((result) => {
      res.json(result.rows[0]);
    })
    .catch(error => {
...
app.delete("/classes/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("DELETE FROM class WHERE id=$1", [id])
    .then(() => {
      res.status(204).json();
    })
    .catch(error => {
...
app.get("/houses", (req, res) => {
  ...
  client.query("SELECT * FROM house ORDER BY name ASC")
    .then((result) => {
      res.json(result.rows);
    })
    .catch(error => {
...
app.get("/houses/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("SELECT * FROM house WHERE id=$1", [id])
    .then((result) => {
      res.json(result.rows[0]);
    })
    .catch(error => {
...
app.delete("/houses/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("DELETE FROM house WHERE id=$1", [id])
    .then(() => {
      res.status(204).json();
    })
    .catch(error => {
...
app.get("/rooms", (req, res) => {
  ...
  client.query("SELECT * FROM room ORDER BY name ASC")
    .then((result) => {
      res.json(result.rows);
    })
    .catch(error => {
...
app.get("/rooms/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("SELECT * FROM room WHERE id=$1", [id])
    .then((result) => {
      res.json(result.rows[0]);
    })
    .catch(error => {
...
app.post("/rooms", (req, res) => {
  ...
  client.query(
    ...
    [room.name, room.building, room.floor, room.number, room.capacity, room.image]
  ).then((result) => {
    res.status(201).json(result.rows[0]);
  })
  .catch(error => {
...
app.put("/rooms/:id", (req, res) => {
  ...
  client.query(
    ...
    [room.name, room.building, room.floor, room.number, room.capacity, room.image, id]
  ).then((result) => {
    res.status(200).json(result.rows[0]);
  })
  .catch(error => {
...
app.delete("/rooms/:id", (req, res) => {
  ...
  const id = req.params.id;
  client.query("DELETE FROM room WHERE id=$1", [id])
    .then(() => {
      res.status(204).json();
    })
    .catch(error => {
